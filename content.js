...
chrome.storage.sync.get("invoiceData", function (data) {
    ...
    for(var i=0; i<rows.length; i++){
        if(rows[i].children[19] != undefined && rows[i].children[19].innerText=="Yes")
            autogenerated++;
    }
    if(rows.length < invoiceData.length+autogenerated){
        addRowButton.click();
    }
    
    var datePickers = document.getElementsByClassName("date-picker hasDatepicker");
    var j = 0;
    for(var i=0; i<invoiceData.length+autogenerated; i++){ 
        if(rows[i].children[19] != undefined && rows[i].children[19].innerText=="Yes"){
            continue;
        }
        // Set project
        ...
        rows[i].children[4].children[1].value = (invoiceData[j]["minutes"]!="00")?invoiceData[j]["minutes"].replace(/^0+/, ''):"0";
